...
Output main(VertexOutput input)
{
    ...
    // Base Color
    const float4 kObjectAmbientColor = gMeshMaterial.Ka /** albedoColor*/ * gObjectMaterial.color;
    const float4 kObjectDiffuseColor = gMeshMaterial.Kd /** albedoColor*/ * gObjectMaterial.color;
    ...
    const float4 useAmbientColor = checkMaximAmbient ? float4(kBaseAmbientColor.rgb * 0.5f, 1.0f) : kBaseAmbientColor;
    ...
    basicSurface.diffuse = gMeshMaterial.Kd * gObjectMaterial.color;
    basicSurface.specular = gMeshMaterial.Ks * gObjectMaterial.color;
            ...
            case DIRECTIONAL:
                {
                    ...
                    finaleColor += CaluclateDirectionalLightColor(gLights[i], gCamera.worldPosition, basicSurface);
